...
-- 
-- This file contains all of the example queries and exercise solutions 
-- from the book.
-- Refer to the book for references to specific databases.
...
-- 
-- Or visit the LeanPub and Amazon sales pages at:
-- https://leanpub.com/self-guided-sql

-- https://amzn.to/3HdqbBH
...
SELECT FirstName, LastName, Company, PostalCode, 
typeof(PostalCode)
FROM customers;
...
SELECT	FirstName, LastName, iif(typeof(Company) = 'null', 
 		'N/A', Company) AS [Company Name]
FROM		Customers;

-- 3.2.12

SELECT FirstName, LastName, Country, 
 	iif(Country IN ('United States', 'Canada'), 
 	'North America', 
 	iif(Country IN ('Germany', 'France', 'Italy'), 
 	'Europe', 'Undefined')
 	) AS Region
FROM Customers;
...
WHEN Country IN ('United States', 'Canada') 
 	THEN 'North America'
WHEN Country IN ('Germany', 'France', 'Italy') 
 	THEN 'Europe'
ELSE 'Undefined'
END AS Region
FROM Customers;
...
SELECT CustomerID, ShipName, OrderDate, ShipVia, Freight
FROM Orders
...
SELECT LastName, FirstName, strftime('%m/%d/%Y', HireDate) AS [Date Hired] 
FROM	employees;

-- 3.3.25

SELECT LastName, FirstName, HireDate, DateTime(HireDate, '+5 years') AS [Five Year Anniv.] 
FROM employees;

SELECT LastName, FirstName, HireDate, DateTime(HireDate, '+0000-03-00') AS [90 Day Review] 
FROM employees;

SELECT LastName, FirstName, HireDate, DateTime(HireDate, 'start of month', '+1 month') AS [Benefits Start Date] 
FROM employees;
...
SELECT LastName, FirstName, ROUND((JulianDay('now') - JulianDay(HireDate)) / 365.25, 1) AS [Years at Company]
FROM employees;
...
SELECT FirstName, LastName, Company
FROM customers
WHERE Company IS NULL OR Length(Company) = 0;
...
SELECT CustomerId, FirstName, LastName, Email, 
substr(Email, 1, instr(Email, '@')) || 'example.com' AS 
[New Email] 
FROM customers;
...
MAX(RequiredDate, ShippedDate) as [Order Closed]
FROM Orders;

-- 3.4.11

SELECT Productid, ProductName, ReorderLevel, 
MIN(ReorderLevel, 20) AS [New Reorder Level]
FROM Products;
...
SELECT c.LastName, c.FirstName, concat(e.LastName, ', ', e.FirstName) AS [Support Rep]
FROM customers c 
...
SELECT e.LastName, e.FirstName, concat(m.LastName, ', ', m.FirstName) as [Reporting to:]
FROM Employees e
...
SELECT cl.*, email, sa.address, sa.address2, sa.postal_code, sa.phone
FROM customer_list cl
INNER JOIN customer c
ON c.customer_id = cl.ID
INNER JOIN store s
...
SELECT cl.*, email, concat(sa.address,' ', sa.address2, ', ' , city.city, '  ', sa.postal_code, ', ', co.country) AS [Store Location], sa.phone as 'Store Phone'
FROM customer_list cl
INNER JOIN customer c
ON c.customer_id = cl.ID
INNER JOIN store s
...
concat_ws(', ',m.LastName, m.FirstName) AS [Reporting to]
FROM Employees e
INNER JOIN Employees m
ON m.EmployeeId = e.ReportsTo;
...
SELECT concat(LastName, ', ', FirstName) AS [Name], Company,
concat_ws(', ', Address, City, State, Country, 
...
rtrim(BirthDate, ' 00:00:00') AS [Birthdate]
FROM	employees;

-- 3.5.19

SELECT concat(LastName, ', ', FirstName) AS [Name], 
ltrim(Phone, '+1 ') AS [Phone]
FROM employees;
...
SELECT i.InvoiceId, i.InvoiceDate, concat(c.LastName, ', ', c.FirstName) AS [Customer], c.Company, c.email
FROM invoices i
...
SELECT f.title, r.rental_date, r.return_date, i.inventory_id 
FROM Sakila.film f
...
SELECT p.ProductName, c.description, s.CompanyName
...
SELECT o.OrderID, c.CompanyName, o.OrderDate, o.ShippedDate, o.Freight
...
SELECT COUNT(*) AS [Invoices], concat(c.LastName, ', ', c.FirstName) AS [Customer]
FROM invoices i
INNER JOIN customers c
...
SELECT CategoryName, ROUND(AVG(UnitPrice), 2) AS [Average Price] 
...
SELECT GenreId, 
ROUND (AVG(Bytes / 1024 / 1024), 2) AS [Avg Size (MB)]
FROM tracks
GROUP BY GenreId;

-- 3.6.8

SELECT t.GenreId, g.name, ROUND(AVG(Bytes / 1024 / 1024), 2) AS [Avg Size (MB)]
FROM tracks t 
INNER JOIN genres g
ON g.GenreId = t.GenreId
...
SELECT MIN(InvoiceDate) AS [First Invoice], BillingCountry
FROM invoices
...
SELECT strftime('%m-%Y', ShippedDate) AS Month, ROUND(SUM(Subtotal), 2) as [Monthly Sales]
...
SELECT OrderID, round(UnitPrice, 2), round(Quantity, 2),
round(Discount, 2), round(Freight) 
FROM invoices
LIMIT 100;

-- 3.6.15

SELECT OrderID, printf('%.2f', UnitPrice), Quantity, printf('%.2f', Discount), printf('%.2f', ExtendedPrice), 
printf('%.2f', Freight)
FROM invoices
LIMIT 100;
...
SELECT printf('%s - %s ($%.2f)', ProductID, ProductName, 
...
SELECT e.EmployeeId, concat(e.FirstName, ' ', e.LastName) as [Employee], count(*) as [Order Count]
FROM Orders o
...
SELECT strftime('%m-%Y', OrderDate) AS [Month], ROUND(SUM(ExtendedPrice + Freight), 2) AS [Month Total]
FROM Invoices
GROUP BY [Month]
ORDER BY OrderDate;
...
WITH [Order Totals] AS
 	...
 	GROUP BY OrderID)
SELECT OrderID, PRINTF("%.2f", OT) AS [Order Total] 
FROM [Order Totals]
ORDER BY OT DESC
LIMIT 20;
...
SELECT strftime('%Y-%m', OrderDate) AS Period, EmployeeId, COUNT(OrderID) as [Order Count]
FROM Orders
...
SELECT c.FirstName, c.LastName, c.Address, c.City, c.State
FROM customers c
INNER JOIN invoices i 
ON i.CustomerId = c.CustomerId
WHERE i.InvoiceId IN (
...
SELECT mt.Period, MAX(mt.OrderCount) AS [Order Count], 
...
CREATE VIEW vTrackListing AS
 SELECT tr.name AS [Track Name],al.title AS Album, 
 ar.name AS Artist, tr.Composer, tr.Milliseconds,    
 tr.UnitPrice
 FROM tracks tr
...
SELECT tr.TrackId, tr.name AS [Track Name], 
 ...
 strftime('%M:%S', Milliseconds / 1000, 'unixepoch') 
 AS [Length], tr.UnitPrice;
...
(TrackID, Track, AlbumID, Album,
 ...
 SELECT tr.TrackId, tr.name, al.AlbumId, al.title,
...
FROM tracks
WHERE abs(Milliseconds - 180000) <= 10000;

-- 3.8.9

CREATE TEMPORARY TABLE PriceAdjusts AS
SELECT ProductID, ProductName, UnitPrice, 
(random() % UnitPrice) AS Adjust
FROM Products;

SELECT ProductID, ProductName, UnitPrice, 
Adjust as [Adjustment], (UnitPrice + Adjust) 
AS [New Price], iif(sign(Adjust) = 1, 'Increase',
sign(Adjust) = -1, 'Reduction', 'No Change') 
AS Status 
FROM PriceAdjusts;
...
c.LastName, c.FirstName, t.[Invoice Total], c.phone, c.email
FROM invoices i
INNER JOIN customers c
ON c.CustomerId = i.CustomerId
INNER JOIN
...
SELECT FirstName, LastName, Address, City, State,  
 Country, PostalCode, "Customer" AS Relationship
FROM customers
UNION
SELECT FirstName, LastName, Address, City, State, 
Country, PostalCode, "Employee" AS Relationship
FROM employees
...
SELECT me.FirstName, me.LastName, me.phone, ne.FirstName, ne.LastName, ne.HomePhone
...
SELECT * 
FROM customers 
WHERE CompanyName LIKE '%snabbk' || char(246) || 'p';

-- 3.9.11

SELECT * 
FROM customers 
WHERE Address LIKE '%' || char(80, 97, 231, 111) || '%';
...
SELECT 	CONCAT(FirstName, " ", LastName) AS [Customer Name], Company, Address, City, State, PostalCode, Country,"Chinook" AS [Vendor]
FROM Chinook.customers
UNION
SELECT 	ContactName, CompanyName, Address, City, Region, PostalCode, Country, "Northwind" AS [Vendor]
FROM Northwind.customers;
...
FROM Northwind.customers
WHERE COALESCE(city, Region, PostalCode) IS NOT NULL
UNION
SELECT DISTINCT ShipCity, ShipRegion, ShipPostalCode 
FROM Northwind.Orders
WHERE COALESCE(ShipCity, ShipRegion, ShipPostalCode) IS NOT NULL
ORDER BY PostalCode;
...
-- 3.11 – Analyzing Data: Window Functions

-- 3.11.1

SELECT row_number() OVER(ORDER BY total DESC) AS Row,
i.CustomerId, LastName, FirstName InvoiceDate, BillingCountry, total
FROM invoices i
INNER JOIN customers c
ON c.CustomerId = i.CustomerId
LIMIT 10;
...
OrderDate) AS Row, CustomerId, LastName, OrderDate, ShippedDate, 
...
ORDER BY OrderDate) AS Row, CustomerId, LastName, OrderDate, ShippedDate, ShipName, ShipCountry  
FROM Northwind.Orders o 
INNER JOIN Northwind.Employees e 
ON e.EmployeeId = o.EmployeeId) 
WHERE Row = 1 
...
SELECT ProductName, CategoryName, UnitPrice, cume_dist() over(PARTITION BY CategoryName ORDER BY UnitPrice)
FROM	Products p
INNER JOIN Categories c
ON c.CategoryID = p.CategoryID;
...
e.FirstName, e.LastName,  count(o.orderid) AS [Order Count], 
...
SELECT oq.OrderID, CustomerId, ShippedDate, Subtotal, first_value(Subtotal) over(PARTITION BY CustomerId) 
AS [First Order] 
FROM [Orders Qry] oq 
INNER JOIN [Order Subtotals] os 
ON os.OrderID = oq.OrderID;
...
SELECT oq.OrderID, CustomerId, ShippedDate, Subtotal, last_value(Subtotal) over(PARTITION BY CustomerId 
ORDER BY Subtotal RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS [Max Order] 
FROM [Orders Qry] oq 
INNER JOIN [Order Subtotals] os  
ON os.OrderID = oq.OrderID;
...
INNER JOIN [Order Subtotals] os
ON os.OrderID = oq.OrderID)
SELECT OrderID, CustomerId, ShippedDate, Subtotal, [Prev Total],
round(((Subtotal - [Prev Total]) / Subtotal) * 100, 2) AS [Pct. Change]
FROM OrderResults;
...
SELECT CompanyName, ContactName, coalesce(HomePage, Fax, Phone) 
...
SELECT track, album, artist, Minutes, strftime('%H:%M:%S', (sum(Milliseconds) over(PARTITION BY Artist ORDER BY Album RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING))  / 1000, 'unixepoch') AS [Artist Total] 
FROM vTrackListing;

2.

SELECT actors, COUNT(*) as Roles, dense_rank() over(order by COUNT(*) desc) AS Place
FROM film_list
GROUP BY actors;

3.

SELECT fl.*, f.release_year, first_value(fl.title) over(PARTITION BY actors ORDER BY release_year) AS [Introductory Role]
FROM film_list fl
INNER JOIN film f
ON f.film_id = fl.FID;

WITH Filmography AS 
(SELECT fl.*, f.release_year, rank() over(PARTITION BY actors ORDER BY release_year) AS [Intro Role]
FROM film_list fl
INNER JOIN film f
ON f.film_id = fl.FID)
SELECT * 
...
(SELECT	ProductID, ProductName, CategoryID, 
 		UnitPrice, max(UnitPrice) over(PARTITION BY  
 		CategoryID) AS [Category Leader] FROM 
 		Northwind.Products)
SELECT	ProductID, ProductName, CategoryName, UnitPrice, 
 		round((UnitPrice * 1.0 / [Category Leader]) * 100, 
 		2) AS [Percent of Lead], [Category Leader]
FROM 		ProductList pl
INNER JOIN 	Categories c
...
WITH MonthlyTotals AS
(SELECT strftime('%Y-%m', ShippedDate) as Period, sum(ExtendedPrice) AS Invoiced
FROM Northwind.Invoices
WHERE ShippedDate IS NOT NULL
GROUP BY Period)
SELECT Period, [Invoiced], round(avg(Invoiced) over(ORDER BY Period ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW), 2) AS [Running Average]
FROM MonthlyTotals;
...
UPDATE employees
SET Birthdate = datetime(BirthDate, '+20 years'), HireDate = datetime(HireDate, '+15 years');
...
SELECT ProductID, ProductName, CategoryID, NULL AS CategoryName,
QuantityPerUnit, UnitPrice, NULL AS UnitsSold
FROM	Products
WHERE Discontinued = 0;
...
SELECT title, description, replace(description, 'Boring', 'Thoughtful') 
...
UPDATE employees
SET Email = concat(FirstName, '_', LastName, '@chinookcorp.org');
...
SELECT * 
FROM Products 
WHERE upper(ProductName) = 'ANISEED SYRUP';
